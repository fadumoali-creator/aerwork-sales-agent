// AerWork LinkedIn Sales Agent — RASKAS TAUSTAFUNKTIO (Netlify Background Function).
// Tiedostonimen "-background" -pääte kertoo Netlifylle että tätä funktiota EI
// suoriteta 10 sekunnin synkronisella aikarajalla, vaan se saa juosta taustalla
// jopa 15 minuuttia. Netlify vastaa kutsujalle heti "202 Accepted" eikä odota
// tätä funktiota valmiiksi, joten tämä funktio EI voi palauttaa vastausta
// suoraan käyttäjälle — se kirjoittaa lopputuloksen Netlify Blobsiin
// (aerwork-chat-jobs -tallentimeen) jobId:n alle, ja chat-status.js-funktio
// lukee sen sieltä kun frontend kysyy sitä pollaamalla.
//
...
// Normaalisti Netlify antaa Blobsille automaattisesti tarvittavat tiedot
// (siteID, token) kun getStore()-kutsu tehdään funktion sisällä. Tällä
// sivustolla tämä automaattinen "zero-config"-tunnistus ei kuitenkaan toimi
// tuotannossa (tunnettu, ajoittainen Netlify-puolen vika riippumatta koodin
// oikeellisuudesta) — siksi annetaan varalla manuaaliset tiedot, jotka luetaan
// ympäristömuuttujista jos ne on asetettu. Jos niitä ei ole asetettu, käytetään
// automaattista tunnistusta kuten ennenkin (ei siis riko mitään jos/kun
// Netlify korjaa automaattisen tunnistuksen).
function blobsOpts() {
...
TYÖSKENTELYAIKA (tekninen reunaehto): tämä vastauksesi käsitellään taustatyönä, jolla on runsaasti aikaa (ei tiukkaa muutaman sekunnin rajaa) - saat siis tehdä tarvittaessa useamman perätyisen haun (esim. prh_lookup JA web_search samassa vastauksessa) kun se aidosti palvelee käyttäjää. Tästä huolimatta älä hae samaa tietoa moneen kertaan ilman syytä (ks. sääntö yllä) äläkä ketjua hakuja loputtomiin - tee juuri niin monta hakua kuin tarpeen, ei enempää.

TIEDONKERUUN LOGIIKKA (seuraa tarkasti, tämä tekee sinusta fiksumman keskustelukumppanin):
1. Kun saat PELKÄN yrityksen nimen ilman henkilön nimeä: tee prh_lookup (virallinen toimiala) ja jos se on luontevaa, myös lyhyt web_search toimitusjohtajan/CEO:n selvittämiseksi samassa vastauksessa. Kysy SAMALLA normaalisti liidin nimeä ja roolia, ellet juuri löytänyt niitä haulla ("Kerro vielä liidin nimi ja rooli, esim. CEO tai HR-johtaja?" tai jos löysit todennäköisen nimen: "Löysin verkosta nimen [nimi], [rooli] — tarkoititko häntä?"). Jos web_search ei löydä mitään luotettavaa parilla kohdennetulla haulla ("<yritys> toimitusjohtaja", "<yritys> CEO LinkedIn"), kerro se suoraan äläkä jää hakemaan loputtomiin.
...
3. ÄLÄ esitä pisteytyskorttia niin kauan kuin liidin nimi JA rooli puuttuvat kokonaan, PAITSI jos käyttäjä nimenomaisesti pyytää pisteytystä puutteellisilla tiedoilla (esim. kirjoittaa "pisteytä") - silloin annat alustavan kortin placeholder-nimellä ("[Nimi puuttuu]").
...
// josta chat-status.js lukee sen kun frontend pollaa. HUOM: getStore() pitää
// kutsua TÄSSÄ, funktion sisällä pyynnön käsittelyn aikana - ei tiedoston
// ylätasolla - koska Netlifyn Blobs-konteksti alustuu vasta pyynnön aikana
// tuotannossa (toimii virheellisesti vain paikallisessa devissä jos kutsutaan
// moduulin latautuessa).
async function saveJob(jobId, data) {
  if (!jobId) return;
  try {
    const jobsStore = getStore('aerwork-chat-jobs', blobsOpts());
    await jobsStore.setJSON(jobId, data);
  } catch (err) {
    // Jos tallennus itsessään epäonnistuu, ei ole enää keinoa kertoa käyttäjälle
    // mitään (taustafunktion vastausta ei näytetä kenellekään) — lokitetaan vain.
...
exports.handler = async (event) => {
  ...
  if (!jobId) {
    // Ei jobId:tä — ei mitään järkevää paikkaa kirjoittaa tulosta, keskeytetään heti.
...
async function runLeadsDb(input) {
  try {
    const store = getStore('aerwork-leads', blobsOpts());
